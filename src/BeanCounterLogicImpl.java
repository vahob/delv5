...
public class BeanCounterLogicImpl implements BeanCounterLogic {
	...
	private ArrayList<Bean>[] slotBean;
	...
	BeanCounterLogicImpl(int slotCount) {
		...
		slotBean = (ArrayList<Bean>[]) new ArrayList[slotCount];
	...
	public int getSlotBeanCount(int i) {
		// TODO: Implement
		if (slotBean.length == 0) {
			return 0;
		}
		if (slotBean[i] == null) {
			return 0;
		}
		return slotBean[i].size();
	}
	...
	public double getAverageSlotBeanCount() {
		...
		for (int i = 0; i < slotBean.length; i++) {
	...
	public void repeat() {
		...
		for (int i = 0; i < this.getSlotCount(); i++) {
			...
			for (int k = 0; k < slotBean[i].size(); k++) {
				if (slotBean[i].get(k) != null) {
					this.remainingBeans.add(slotBean[i].get(k));
				}
			}
			// this.remainingBeans
			// .addAll(this.slotBean.get(i).stream().filter(x -> x !=
			// null).collect(Collectors.toList()));
			if (this.inFlightBeans[i] != null) {
				this.remainingBeans.add(this.inFlightBeans[i]);
			}
		}
		for (int i = 0; i < getSlotCount(); i++) {
			ArrayList<Bean> e = new ArrayList<>();
	...
	private void insertBean() {
		...
		if (!remainingBeans.isEmpty()) {
			...
			this.inFlightBeans[0] = this.remainingBeans.remove(0);
		} else {
			this.inFlightBeans[0] = null;
		}
	}
